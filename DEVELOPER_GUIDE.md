...
### 5.1 Current state: manual deploy to Cloudflare Workers

There is no automated CI/CD pipeline running today. Deployment is a manual, local command:
...
python3 -c '... patch .output/server/wrangler.json: preview_urls = false ...'
npx wrangler --cwd .output/ deploy               # actually publishes to Cloudflare
```

**Why the Python patch step exists:** Cloudflare auto-enables an extra public "Preview URL" hostname per deployment unless `preview_urls: false` is set in `wrangler.json` — but that file is entirely regenerated by Nitro on every build (it's derived from `vite.config.ts` + the Nitro preset, not a source file you own), so the flag can't just be committed once. The script re-applies it after every build, before deploying.

**Prerequisites to run this yourself:** `npx wrangler login` once (opens a browser, authenticates against your Cloudflare account — this is a one-time interactive step that can't be scripted/automated in CI without setting up an API token instead).

### 5.2 Why not GitHub Actions / real CI?

There *is* a workflow file, `.github/workflows/deploy-pages.yml`, but it targets **GitHub Pages**, not Cloudflare, and it's currently non-functional — see §5.3. Nothing currently automates the Cloudflare deploy path in CI. If you want that, the shape would be:
...
- run: bun run build
- run: npx wrangler deploy --cwd .output/
  env:
...
```
(plus the same `preview_urls` patch step). Not implemented — the app is small enough that manual `scripts/deploy.sh` has been sufficient, and setting up a scoped Cloudflare API token is a deliberate decision to make consciously, not something to wire up silently.
