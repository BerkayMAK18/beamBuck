...
- **Change what appears in the Gallery** — the Gallery filters to `status = 'done'` in `src/routes/_authenticated/gallery.tsx`. Swap the filter if you want a different curation rule.
...
## 4. Storage paths (Gallery photos + avatars)

**Buckets are `bucket-photos` (Gallery / bucket-item photos) and `avatars` (profile pictures).** Both are private; the app generates 365-day signed URLs on upload and persists those URLs into the DB.

### Gallery / bucket-item photo uploads

**File:** `src/routes/_authenticated/gallery.tsx`
...
- **Longer / shorter signed URL lifetime** — change the second arg to `createSignedUrl` (in seconds). Note: once the URL is stored in `bucket_items.image_urls`, it doesn't refresh; when the signature expires, images 404. If you need long-term URLs, either use `getPublicUrl` or add a re-sign step on read.
...
Both `bucket.tsx`, `calendar.tsx`, and `gallery.tsx` subscribe to `postgres_changes` on `bucket_items`:
...
| What appears in Gallery | `src/routes/_authenticated/gallery.tsx` (Supabase query filters) |
| Photo upload path | `src/routes/_authenticated/gallery.tsx` → `uploadPhotos` |
